...
export class SlideGen extends HTMLElement {
    ...
    slideParam:slideParam = {} //overall param for all slides
    slideParams:slideParam[] = [] // each param for each slide (slide param 1 == index 0)
    slideArr:any = [] //container for all slides
    miniSlideBtnArr:any = [] //container for all slides miniBtns
    miniBtnIcons:any = []
    currentSlide = 0
    slideSpace:any = 0
    ...
    //main nodes
    mainCont:any = document.createElement("ul")
    slideCont:any = document.createElement("div")
    prevBtn:any = document.createElement("button")
    nextBtn:any = this.prevBtn.cloneNode()
    miniSlideBtns:any = this.mainCont.cloneNode()

    //button nodes
    prevImg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    nextImg:any = this.prevImg.cloneNode()
    ...
    prevBtnFunc = (offset?:number,temp?:boolean)=>{ //going to Left
        if(temp && typeof offset == "number"){
            ...
            } else this.slideCont.yDeg += this.slideSpace
    ...
    nextBtnFunc = (offset?:number,temp?:boolean)=>{ //going to Right
        if(temp && typeof offset == "number"){
            // console.log(this.slideCont.yDeg - offset)
            this.addPEAS(this.slideCont,{
                styles:{
    ...
    async setupSlideContainer(obj:{
        ...
        //temp config setup
        if(obj.config.slideBtns == false) this.config.slideBtns = false
        if(obj.config.miniSlideBtns == false) this.config.miniSlideBtns = false
            ...
            props:{
                ...
                onSelect:()=>{
                    ...
                    this.prevBtn.loop = setInterval(()=>{
                        ...
                        } else clearInterval(this.prevBtn.loop)
                    },350)
                },
                onActive:()=>{
                    this.addPEAS(this.prevBtn,{styles:{"background":this.uiColor[500],scale:"1 1 1"}})

                    if(!this.prevBtn.looping && this.prevBtn.isDown)this.prevBtnFunc()
            ...
            props:{
                ...
                onSelect:()=>{
                    ...
                    this.nextBtn.loop = setInterval(()=>{
                        ...
                        } else clearInterval(this.nextBtn.loop)
                    },350)
                },
                onActive:()=>{
                    this.addPEAS(this.nextBtn,{styles:{"background":this.uiColor[500],scale:"1 1 1"}})

                    if(!this.nextBtn.looping && this.nextBtn.isDown) this.nextBtnFunc()
    ...
    setupMiniSlideBtns(){
        ...
        for(let i=0; i<this.totalSlides;i++){
            //gen mini slide btns
            let miniBtnCont:any = document.createElement("div")
            ...
            this.addPEAS(miniBtnCont,{
                ...
                events:{
                    ...
                    onActive:()=>{
                        if(miniBtn.isDown) miniBtnCont.navigate()
    ...
    setupSlides(){
        ...
        for(let i=0; i<this.totalSlides;i++){
            let slide:any = document.createElement("div")
            ...
            this.addPEAS(slide,{
                ...
                events:{
                    ...
                    onSelectMove:(e:any)=>{ //temp movement for slide drag
                        slide.isDown = true
                    ...
                    onActive:(e:any)=>{ //move on certain drag length else reset
                        this.addPEAS(this.slideCont,{
                            styles:{
                    ...
                    onLeave:(e:any)=>{ //move on certain drag length else reset
                        this.addPEAS(this.slideCont,{
                            styles:{
                                "transition-duration":this.animDur
                            }
    ...
    setupCarousel(){
        let evenSpaceInDeg:any = this.slideSpace =  (360/this.totalSlides)
        ...
        let posArr:any =[] //slidePosArr
        for(let i=0;i<this.slideArr.length;i++){
            posArr.push(i)
            let adjustCircCoordNRotation =(deg?:number,move?:boolean)=>{ //unique function for each element
                if(typeof deg == "number") this.slideArr[i].coordConfig.coordDeg = deg.toFixed(2)
            ...
            if(i == 0) this.slideArr[i].dataset.active = true
