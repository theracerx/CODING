...
interface stationPlayer extends HTMLAudioElement{
    station: station2
    stationBackup:any
    chk:{
        music:boolean
        commercial:boolean
        commercialSeq:boolean
        musicSeq:boolean
    }
    stats:{
        musicIDMax: number | undefined, 
        commercialIDMax: number | undefined,
        commercialSeqIDMax: number | undefined, 
        musicSeqIDMax: number | undefined, 
        nextCommercialIn: number | undefined, //for initial display
    }
    //---------Play Methods
    randomizePlay:any //decides what to play
    ...
    // pauseRadioStation?:any
    // nextRadioStation?:any
    playStation:any, //dflt play
    pauseStation:any, //dflt pause
    endStation:any, //dflt end
    waitStation:any, //dflt wait
    nextStationAudio:any
    
    //---------MISC
    nowPlaying?:any
    // AVOID "ANY" DATA TYPE AS MUCH AS POSSIBLE
...
interface radioControl extends HTMLElement{
    src?:string
    value?:any
    on?:any
    off?:any
...
/* 
    onRadioReady:   //fires when all audio tags are generated
    onRadioPlay:    //fires when audio is playing
    onRadioPause:   //fires when audio is paused
    onRadioWait:    //fires when audio is waiting

*/

declare global{
    interface HTMLElementTagNameMap{
        "radio-player" : AudioRadio
    }
}

interface AudioRadioInterface extends HTMLElement {
    cStation: number //index of current active Station
    stations: any | station[]
    stationPlayers:stationPlayer[]
    onRadioReady:any //fires when all audio tags are generated
    radioReady:any //event for onRadioReady
    onRadioPlay:any //fires when audio is playing
    radioPlay:any //event for onRadioPlay
    onRadioPause:any //fires when audio is paused
    radioPause:any //event for onRadioPause
    onRadioWait:any //fires when audio is waiting
    radioWait:any //event for onRadioWait
    onRadioEnd:any //fires when audio is waiting
    radioEnd:any //event for onRadioEnd
    isInterupted:boolean
    setupRadio():void
    setupStation(station:station):void
    setupRadioControls():void
    interrupt():void
    resume():void
}

export class AudioRadio extends HTMLElement implements AudioRadioInterface{

    
    cStation = 0
    stations: any | station[] //no value because will be added later
    stationPlayers:stationPlayer[] //no value because will be added later
    ...
    */
    setupRadio(obj?:radioControls){ //targets an element
    // setupRadio(stations:station[],obj?:radioControls){ //targets an element
        ...
        this.stations.forEach(async (station:station)=>{await this.setupStation(station)})
        ...
        } else throw new Error("audio elements doesn't match number of")
        
    }
    setupStation(station:station){
        return new Promise((resolve:any,reject:any)=>{
            let audio:stationPlayer = Object.assign(document.createElement("audio"),{
                ...
                randomizePlay:(standby?:boolean)=>{ //decides what to play
                    let forceRoute = ()=>{
                    ...
                    let commercialSeqRoute = ()=>{ //ads then music
                    ...
                    let musicSeqRoute = ()=>{ //music then ads
                    ...
                    if (audio.station.queue.length > 0){ forceRoute()}
                        ...
                        if(audio.chk.commercial && audio.chk.commercialSeq){ Math.round(Math.random()) ? commercialRoute() : commercialSeqRoute()
                        } else if(audio.chk.commercial){commercialRoute()
                        } else if(audio.chk.commercialSeq){commercialSeqRoute()
                        } else throw new Error("something went wrong playing commercial")
                ...
                addQueue:(list:any,type:string)=>{ //used by CommercialSeq & MusicSeq for ForceRoute
                ...
                shuffleStation:(type:string)=>{ //creates randomize array
                ...
                endStation:()=>{
                    ...
                    setTimeout(()=>{ //add delay on next audio
    ...
    setupRadioControls(obj?:radioControls){
        /* 
            if no buttons provided then generate
            if provided then apply events
            
        */
        let nextBtn:radioControl,
        prevBtn:radioControl,
        stationName:radioControl,
        volume: radioControl,
        muteBtn:radioControl
    ...
    interrupt(){ //force stop and prevents play until resume() is called
...
let btnOptions = {
    ...
    addTrigger:(target:any)=>{
        target.addEventListener("pointerup",()=>{
            if(Array.isArray(radio.stations) && radio.stations.length > 0){
                radio.setupRadio()
                target.remove()
            } else throw new Error("<radio-player>.stations must not be empty")
        })
    }
}
