...
interface stationPlayer extends HTMLAudioElement{
    ...
    //---------Reset Methods
    selectRandomFromArr(arr:any[]):any
    selectRandomsFromArr(arr:any[],num:number):void
    ...
    timeUpdateStation:any // dflt time update
...
interface AudioRadioInterface extends HTMLElement {
    ...
    radioReady:any // same audio event but fired to radio
    radioPlay:any
    radioPause:any 
    radioWait:any 
    radioEnd:any 
    radioDurationChange:any 
    radioRateChange:any
    radioLoadStart:any 
    radioVolumeChange:any 
...
export class AudioRadio extends HTMLElement implements AudioRadioInterface{
    ...
    attributeChangedCallback(attr:string, oldValue:any, newValue:any) { //CLEAR
    ...
    */
    async setupRadio(obj?:radioControls){ //targets an element
    // setupRadio(stations:station[],obj?:radioControls){ //targets an element
        ...
        */
        ...
        if(this.getElementsByTagName("audio").length == this.stations.length){
            ...
            let initialStation:any = this.getAttribute("station")
    ...
    async setupStation(station:station){
        return new Promise(async(resolve:any)=>{
            let audio:stationPlayer = Object.assign(document.createElement("audio"),{
                ...
                updateStats:(src:string,type?:string)=>{
                    /* 
                        document the src
                        update stats
                    */
                ...
                endStation:()=>{
                    ...
                    setTimeout(()=>{ //add delay on next audio
                        if(prevStation != this.cStation) return
    ...
    testdelay(txt?:any){
        return new Promise((resolve:any)=>{
            setTimeout(()=>{
                if(txt)console.log("finished delay on: " + txt)
                resolve()
            },1500)
        })
    }
